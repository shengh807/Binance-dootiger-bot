...
class Strategy(AutoTrader):  # AutoTrader 로 상속받아 생성된 전략파일
    ...
    def cell_coin_realtime(self, coin: Coin):
        ...
        if cache_price > self.account.total_coin_price * 1.007:
    ...
    def buy_coin(self, coin: Coin):
        ...
        if self.manager.backtest_yn == True:
            coin_price_list_5 = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "5m",
                                                            self.manager.datetime - timedelta(minutes=5), 30)
            coin_price_list_30 = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "30m",
                                                             self.manager.datetime - timedelta(minutes=30), 30)
        else:
            coin_price_list_5 = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "5m",
                                                            datetime.now(), 30)
            coin_price_list_30 = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "30m",
        ...
        if score >= 60:
            ...
            self.manager.buy_alt(self.strategy_name, coin, self.config.BRIDGE,
                                 float(self.BUY_STD_USDT_BALANCE))  # 1000 USDT
    ...
    def cell_coin(self, coin: Coin):
        ...
        if self.manager.backtest_yn == True:
            coin_price_list = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "5m",
                                                          self.manager.datetime - timedelta(minutes=5), 5)
        else:
            coin_price_list = self.db.get_coin_price_list(coin.symbol + self.config.BRIDGE.symbol, "5m", datetime.now(),
                                                          10)
        ...
        ###################### 손절 ######################
        if coin_price_list[0].close < account.total_coin_price * 0.997 \
        ...
        if account.total_coin_balance > 0 \
                and self.manager.balances[self.strategy_name]['buy_more_cnt'] == 0 \
                and son_datatime >= self.account.datetime + timedelta(minutes=30):
            self.logger.info("물타기 전 시간초과 손절 매도신호 감지되었습니다...")
            self.manager.balances[self.strategy_name]['buy_more_cnt'] = 0
            self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 100)

        ##################### 추매물량 일부정리 ######################
        if coin_price_list[0].close > account.total_coin_price and self.manager.balances[self.strategy_name][
            'buy_more_cnt'] > 0:
    ...
    def get_score_ohlcv_now(self, coin_price_list_5: list, coin_price_list_30: list):
        ...
        # 1. 가격 30분봉 비교
        ########################################################################
        score_2 = self.scoring.get_price_over_down_cnt_score2(coin_price_list_30)
        result_score += score_2

        ########################################################################
        # 2. RSI 점수
        ########################################################################
        score_5 = self.scoring.get_price_to_rsi_score3(coin_price_list_30)
        result_score += score_5

        self.logger.warning(f"===== Report [{coin_price_list_30[0]['datetime']}] =====\n"
                            f"1. 가격 30분봉 비교 :::: {score_2}\n"
                            f"2. RSI 점수 :::: {score_5}\n"
                            )
