...
class Strategy(AutoTrader):     # AutoTrader 로 상속받아 생성된 전략파일
    ...
    def cell_coin_realtime(self, coin: Coin):
        ...
        ###################### 부분익절 ######################
        if cache_price < self.account.total_coin_price * 0.993 \
    ...
    def cell_coin(self, coin: Coin):
        ...
        ###################### 부분익절 ######################
        if coin_price_list[0].close < account.total_coin_price * 0.993 \
                and self.manager.balances[self.strategy_name]['sell_more_cnt'] == 0:
            # 현재 가지고있는 코인의 per 만큼 매도한다
            self.logger.info("부분익절 매도신호 감지되었습니다...")
            self.manager.balances[self.strategy_name]['sell_more_cnt'] = 1
            if self.manager.backtest_yn == True:
                self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 50, account.total_coin_price * 0.993)
            else:
                self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 50)

        ###################### 익절 ######################
        if coin_price_list[0].close < account.total_coin_price * 0.985 \
                and self.manager.balances[self.strategy_name]['sell_more_cnt'] == 1:
            # 현재 가지고있는 코인의 per 만큼 매도한다
            self.logger.info("익절 매도신호 감지되었습니다...")
            self.manager.balances[self.strategy_name]['sell_more_cnt'] = 0
            if self.manager.backtest_yn == True:
                self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 100, account.total_coin_price * 0.985)
            else:
        ...
        if coin_price_list[0].close > account.total_coin_price * 1.005:
            ...
            self.manager.balances[self.strategy_name]['sell_more_cnt'] = 0
            if self.manager.backtest_yn == True:
                self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 100, account.total_coin_price * 1.005)
            else:
                self.manager.sell_alt(self.strategy_name, coin, self.config.BRIDGE, 100)
    ...
    def get_score_ohlcv_now(self, coin_price_5m_realtime: list, coin_price_list_30: list, cache_price=0):
        ...
        ########################################################################
        score_1 = self.scoring.get_price_smaller_then_bf10_score(coin_price_list_30, cache_price)
        ...
        self.logger.warning(f"=== Report [{coin_price_list_30[0]['datetime']}]===\n"
                            f"1. 현재 가격 30분봉 비교(하락) :::: {score_1}\n"
                            f"2. 현재 거래량 대비 직전봉(30분)비교 :::: {score_2}\n"
                            )
