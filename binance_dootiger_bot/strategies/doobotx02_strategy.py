...
class Strategy(AutoTrader):     # AutoTrader 로 상속받아 생성된 전략파일
    ...
    def buy_more_coin(self, coin: Coin):
        ...
        if change < -0.02 * (self.manager.balances[self.strategy_name]['buy_more_cnt'] + 1) \
                and down_tail > up_tail * (self.manager.balances[self.strategy_name]['buy_more_cnt'] + 2) \
    ...
    def cell_coin(self, coin: Coin):
        ...
        if coin_price_list[0].close < account.total_coin_price * 0.995:
            ...
            # 물타고 3개봉 지켜보기 룰 추가
            # if son_datatime > self.account.datetime + timedelta(minutes=15):
        ...
        if account.total_coin_balance > 0 \
                and son_datatime >= self.account.datetime + timedelta(minutes=240):
    ...
    def get_score_ohlcv_now(self, coin_price_list: str):
        ...
        # 1. 하락 갯수
        ########################################################################
        score_1 = self.scoring.get_price_down_cnt_score(coin_price_list)
        result_score += score_1

        ########################################################################
        # 2. 급 하락 갯수
        ########################################################################
        score_2 = self.scoring.get_price_over_down_cnt_score(coin_price_list)
        result_score += score_2

        ########################################################################
        # 3. 거래량 급등 점수
        ########################################################################
        score_3 = self.scoring.get_volume_change_before_score(coin_price_list)
        ...
        # 6. RSI 점수
        ########################################################################
        score_6 = self.scoring.get_price_to_rsi_score2(coin_price_list)
        result_score += score_6

        self.logger.warning(f"===== Report [{coin_price_list[0]['datetime']}] =====\n"
                            f"1. 하락봉(6개▲/10개) :::: {score_1}\n"
                            f"2. 급하락봉(2개▲/10개) :::: {score_2}\n"
                            f"3. 거래량급등(3배▲[10], 10배▲[20]) :::: {score_3}\n"
                            f"4. 이평선이격(▼[10]) :::: {score_4}\n"
                            f"5. 볼린저밴드위치(30▼[10], 70▲[-10]) :::: {score_5}\n"
                            f"6. RSI(30▼[10], 70▲[-10]) :::: {score_6}")
